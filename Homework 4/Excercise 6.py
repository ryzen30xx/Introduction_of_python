...
random_request = int(input("How many times do you want to random:")); lowest = int(input("Enter min number: ")); highest = int(input("Enter max number: ")); dict = []
for i in range(random_request):
    random_number = randint(lowest, highest)
    if random_number == 0: print("Bad luck, no random values for you"); exit()
